...
describe('GET /todos/:id', () => {
    ...
    it('should return not return todo doc creaetd by other user', (done) => {
        request(app)
    ...
    it('should return 404 if todo not found', (done) => {
        var testId = new ObjectID();  // a vailid but non existing id
        request(app)
    ...
    describe('DELETE /todos/:id', () => {
        ...
        it('should not remove a todo owned by other user', (done) => {
            var hexId = todos[0]._id.toHexString(); // a different user
    ...
    describe('PATCH /todos/:id', () => {
        it('should update the todo', (done) => {
            ...
            const changed = 'Some updated text';
            body = { text: changed, completed: true };
            request(app)
                .patch(`/todos/${hexId}`)
                .set('x-auth', users[0].tokens[0].token)
                .send(body)
        ...
        it('should not update todo owned by 2nd user', (done) => {
            var hexId = todos[0]._id.toHexString();
            const changed = 'Some updated text';
            body = { text: changed, completed: true };
            request(app)
        ...
        it('should clear completedAt when todo is not completed', (done) => {
            var hexId = todos[1]._id.toHexString();
            const changed = 'Some updated text';
            body = { text: changed, completed: false };
            request(app)
                .patch(`/todos/${hexId}`)
                .set('x-auth', users[1].tokens[0].token)
                .send({
...
describe('POST /users', () => {
    ...
    it('should return validation errors is request invalid', done => {
        // send invalid email and expect 400
    ...
    it('should not create user if email in use', done => {
        // send email already in use and expect 400
